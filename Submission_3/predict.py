...
def get_optimal_threshold(probs,class_id):
    ...
    print(class_id,f"q25: {q25:.4f}, q75: {q75:.4f}, skewness: {skewness:.4f}, median: {median:.4f}")

    # Determine threshold based on distribution characteristics
    if skewness > 1: 
        threshold = max(0.5, q25)  
    elif skewness <= -2.5:
        sorted_probs = np.sort(probs)
        if len(sorted_probs) >= 185:
            threshold = sorted_probs[-185]
        else:
            threshold = 1
    elif skewness <= -2 and skewness > -2.5:
        sorted_probs = np.sort(probs)
        if len(sorted_probs) >= 150:
            threshold = sorted_probs[-150]
        else:
            threshold = 1
    elif skewness <= -1.9 and skewness > -2 :
        sorted_probs = np.sort(probs)
        if len(sorted_probs) >= 100:
            threshold = sorted_probs[-100]
        else:
            threshold = 1
    elif skewness >= -1.9 and skewness <= -1.0:
        sorted_probs = np.sort(probs)
        if len(sorted_probs) >= 20:
            threshold = sorted_probs[-20]
        else:
            threshold = 1
        threshold = 0.9781
    elif -1.0 < skewness <= 0:
        if median > 0.81:
            ...
            threshold = 0.9 * (normalized_skewness ** 2)
...
for class_id, probs in class_probabilities.items():
    class_thresholds[class_id] = 0.9781  # Default threshold
    class_thresholds[class_id] = get_optimal_threshold(probs,class_id)
    # Count elements with probability greater than 0.9781
    count_above_threshold = sum(p > 0.9781 for p in probs)
